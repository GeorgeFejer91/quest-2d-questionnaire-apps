...
public final class ChainLaunchContractTest {
    ...
    @Test
    public void explicitUnityTriggerResolvesQuestionnaireOwnedSequence() throws Exception {
        QuestionnaireData.RuntimeConfig config = QuestionnaireData.RuntimeConfig.fromJson(
            "{"
        ...
        Intent launcher = new Intent(Intent.ACTION_MAIN);
        QuestionnaireLaunchContext launcherContext = QuestionnaireLaunchContext.fromIntent(launcher, config);
        assertEquals("demographics", launcherContext.questionnaireSequenceCsv());
