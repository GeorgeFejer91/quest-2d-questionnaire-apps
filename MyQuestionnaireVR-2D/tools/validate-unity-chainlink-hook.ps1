...
Add-Check -Name 'ChainLink package constant' -Pass (Test-Text $bridgeText 'ChainLinkPackage\s*=\s*"org\.questquestionnaire\.chainlink"') -Detail 'Bridge targets ChainLink package.'
Add-Check -Name 'ChainLink activity constant' -Pass (Test-Text $bridgeText 'ChainLinkActivity\s*=\s*"org\.questquestionnaire\.chainlink\.ChainLinkActivity"') -Detail 'Bridge targets ChainLink activity.'
Add-Check -Name 'ChainLink command action' -Pass (Test-Text $bridgeText 'ChainLinkCommandAction\s*=\s*"org\.questquestionnaire\.chainlink\.COMMAND"') -Detail 'Bridge uses command action.'
...
Add-Check -Name 'README documents command contract' -Pass (Test-Text $readmeText 'org\.questquestionnaire\.chainlink\.COMMAND.*?mq\.command\s*=\s*nextBlock') -Detail 'README states ChainLink command contract.'

$failed = @($checks | Where-Object { -not $_.pass })
$summary = [ordered]@{
    schemaVersion = 'questquestionnaire.unity-chainlink-hook-validation.v1'
