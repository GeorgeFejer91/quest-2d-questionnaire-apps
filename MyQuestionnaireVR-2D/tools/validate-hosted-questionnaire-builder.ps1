...
$requirements = @(
    ...
    [ordered]@{ id = 'operator-guardrail-receipts-capability'; text = 'operator-guardrail-receipts' },
    ...
    [ordered]@{ id = 'manual-signoff-guardrail-text'; text = 'stop conditions checked: controller dialog, start gate, frozen video, no Meta/ADB recovery' },
    [ordered]@{ id = 'physical-packet-guardrail-text'; text = 'operator guardrails included: 2D-first start gate, no controller dialog, no Meta/ADB recovery, video resumes' },
